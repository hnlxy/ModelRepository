...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.dwc = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=3,
                             groups=num_heads, padding=3 // 2)
    ...
    def forward(self, x):
        ...
        out = rearrange(out, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=h, w=w) + feature_map
...
class CCABlock(nn.Module):
    ...
    def __init__(self, channel=512, reduction=16):
        ...
        self.coor = CoordAtt(inp=channel, oup=channel, reduction=reduction)
...
class IDTransformer(nn.Module):
    def __init__(self,
        ...
        self.cca1 = CCABlock(channel=int(dim * 2 ** 2))
        ...
        self.cca2 = CCABlock(channel=int(dim * 2 ** 1))
