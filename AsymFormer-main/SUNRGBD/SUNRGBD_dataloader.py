...
class SUNRGBD(Dataset):
    def __init__(self, transform=None, phase_train=True, data_dir=None, txt_name='train.txt'):
        ...
        if txt_name == 'train.txt':
            image_dir = os.path.join(root, 'train_image')
            depth_dir = os.path.join(root, 'train_depth')
            mask_dir = os.path.join(root, 'train_label')
        elif txt_name == 'test.txt':
            image_dir = os.path.join(root, 'test_image')
            depth_dir = os.path.join(root, 'test_depth')
            mask_dir = os.path.join(root, 'test_label')
    ...
    def __getitem__(self, idx):
        ...
        label = cv2.imread(label_dir[idx], flags=0)
        depth = cv2.imread(depth_dir[idx], flags=-1)
