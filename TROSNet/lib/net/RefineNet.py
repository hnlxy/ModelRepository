...
class RefineNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.upsample(out, imsize, mode='bilinear', align_corners=True)
        return out


def rf101(pretrained=True, **kwargs):
    model = RefineNet(Bottleneck, [3, 4, 23, 3], num_classes=3, **kwargs)
    if True:
        pretrained_dict = torch.load(weight_dir_resnet_101)
