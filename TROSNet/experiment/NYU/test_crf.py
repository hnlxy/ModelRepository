...
from utils.visualize import colored_segmap, colored_heatmap, colored_depthmap
    ...
    def test(self):
        ...
        for i, sample in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                output = self.crf(output, image, depth)
            ...
            depth = sample['depth_%f' % (1.0)][0, 0, :, :].data.cpu()
            ...
            merge = np.hstack([rgb, colored_segmap(gt_label, cfg.MODEL_NUM_CLASSES),
                               colored_segmap(pred_label, cfg.MODEL_NUM_CLASSES)])
