...
from utils.losses import SegmentationLosses, calculate_weigths_labels
...
class Trainer:
    def __init__(self):
        ...
        if cfg.TRAIN_LOSS_BALANCE:
            ...
            if os.path.isfile(classes_weights_path):
                ...
                print("=> Calculating loss balanced weights ...")
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(self.train_loader):
            ...
            target = [sample[s].to(self.device) for s in self.label_list]

            # b, c, h, w = image.shape
            # image = torch.zeros(b, c, h, w)
            
            # Learning Rate
            vLR=(0.001-0.0004*i/150)+(0.0005-0.0002*i/150)*sin(self.iteration/228*2*pi)
            self.optimizer.param_groups[0]['lr']=vLR

            self.optimizer.zero_grad()
            output = self.model(rgb = image, depth = depth, phase = 'train')
    ...
    def val(self, epoch):
        ...
        for i, sample in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                output = self.model(rgb = image, depth = depth, phase = 'val')
