...
def test_model_on_dataset(model, dataset):
    ...
    with torch.no_grad():
        ...
        if type(dataset).__name__ == 'ImageNet2p':
            ...
            try:
                if len(dataset.sampler.indices) <= 1000:
                    print("[Warning] Dataset has fewer than 1000 samples. Evaluation may be unreliable.")
                else:
                    sample = dataset.train_dataset.__getitem__(dataset.sampler.indices[1000])
                    if not isinstance(sample, dict) or 'image_paths' not in sample:
                        print("[Warning] Sample does not contain 'image_paths'. Evaluation may be incomplete.")
            except Exception as e:
                print(f"[Warning] Could not verify sample integrity: {e}")
